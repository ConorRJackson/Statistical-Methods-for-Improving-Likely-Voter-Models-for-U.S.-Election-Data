### 2016 Key State subset Logistic Regression ###
...
x.state<-x
x.state$result[x.state$CL_matched=="Y"]<-0
x.state$result[x.state$CL_E2016GVM=="unknown"]<-1
x.state$result[x.state$CL_E2016GVM=="polling"]<-1
x.state$result[x.state$CL_E2016GVM=="earlyVote"]<-1
x.state$result[x.state$CL_E2016GVM=="absentee"]<-1
x.state$result[x.state$CL_E2016GVM=="mail"]<-1
x.state$result<-factor(x.state$result,levels=c(0,1))
x.state<-x.state[,c("newsint","CC16_327","CC16_364","birthyr","gender","employ","race",
        "educ","marstat","pew_religimp","child18","ideo5","faminc","immstat",
        "ownhome","CC16_361","CC16_360","inputstate","result")]
...
x.state$birthyear<-2016-x.state$birthyear
...
glm.probs.Michigan <- predict(fit.glm.Michigan,
                              newdata = x.state[indtest.s.Michigan,],
                              type = "response")
